...
public class ScoreTrend extends View {
    ...
    private int[] score = new int[]{110, 225, 441, 4845, 552, 668, 660, 663, 669, 678, 682, 689};
    ...
    private void init() {
        ...
        straightPaint.setColor((straightLineColor));//设置刻度线
        ...
        dottedPaint.setColor((straightLineColor));//设置虚线
    ...
    private boolean validateTouch(float x, float y) {
        ...
        //月份触摸区域
        //计算每个月份X坐标的中心点
        float monthTouchY = viewHeight * 0.7f - dipToPx(4);//减去dipToPx(3)增大触摸面积
    ...
    private void drawText(Canvas canvas) {
        textPaint.setTextSize(dipToPx(15));
        textPaint.setColor(textNormalColor);//设置最大值和最小值文本
        ...
        float newWith = viewWith - (viewWith * 0.15f) * 2;//分隔线距离最左边和最右边的距离是0.15倍的viewWith
        float coordinateX;//分隔线X坐标
        textPaint.setTextSize(dipToPx(13));
        textPaint.setStyle(Paint.Style.FILL);
        textPaint.setColor(textNormalColor);
    ...
    private void drawFloatTextBackground(Canvas canvas, int x, int y) {
        brokenPath.reset();
        brokenPaint.setColor(brokenLineColor); //这个是显示折线上浮动框的颜色值
        brokenPaint.setStyle(Paint.Style.FILL);
